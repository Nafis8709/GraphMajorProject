...
int main(){
    ...
    double totalCarCost = 0, totalMetroCost = 0, totalBusCost = 0;
    double carDist = 0, metroDist = 0, busDist = 0;
    ...
    for(int i = 0; i < path.size(); i++){
        ...
        if(i < path.size() - 1){
            ...
            if(G.nodes[nodeId].type == "road" && G.nodes[nextNode].type == "road"){
                ...
                mode = "bus";
                segCost = segDist * 7.0;
                totalBusCost += segCost;
                busDist += segDist;
            }
            
            cout << " -> " << fixed << setprecision(3) << segDist << " km (" << mode << ", ৳" 
    ...
    cout << "Bus distance: " << fixed << setprecision(3) << busDist << " km (Cost: ৳" 
         << setprecision(2) << totalBusCost << ")" << endl;
    cout << "Total cost: ৳" << fixed << setprecision(2) << (totalCarCost + totalMetroCost + totalBusCost) << endl;
